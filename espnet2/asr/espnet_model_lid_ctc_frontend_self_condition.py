...
class ESPnetASRLIDCTCSelfConditionFrontendModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        ctc: CTC,
        frontend_interctc: Optional[CTC],
        joint_network: Optional[torch.nn.Module],
        aux_ctc: Optional[dict] = None,
        frontend_aux_ctc: Optional[dict] = None,
        ctc_weight: float = 0.5,
        interctc_weight: float = 0.0,
        frontend_interctc_weight: float = 0.0,
        ignore_id: int = -1,
        ...
        self.frontend_interctc_weight = frontend_interctc_weight
        self.aux_ctc = aux_ctc
        self.frontend_aux_ctc = frontend_aux_ctc
        ...
        self.frontend_interctc = frontend_interctc
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 1. Encoder
        encoder_out, encoder_out_lens, frontend_intermediate_outs = self.encode(speech, speech_lengths, self.frontend_interctc)
        ...
        # Frontend Intermediate CTC (optional)
        frontend_loss_interctc = 0.0
        if self.frontend_interctc_weight != 0.0 and frontend_intermediate_outs is not None:
            for layer_idx, frontend_intermediate_out in enumerate(frontend_intermediate_outs):
                # we assume frontend_intermediate_out has the same length & padding
                # as those of encoder_out
                idx_key = str(layer_idx)
                # use auxillary ctc data if specified
                loss_ic = None
                if self.frontend_aux_ctc is not None and idx_key in self.frontend_aux_ctc:
                    aux_data_key = self.frontend_aux_ctc[idx_key]
                    aux_data_tensor = kwargs.get(aux_data_key, None)
                    aux_data_lengths = kwargs.get(aux_data_key + "_lengths", None)

                    if aux_data_tensor is not None and aux_data_lengths is not None:
                        loss_ic, cer_ic = self._calc_frontend_interctc_loss(
                            frontend_intermediate_out,
                            encoder_out_lens,
                            aux_data_tensor,
                            aux_data_lengths,
                        )
    ...
    def encode(
        self, speech: torch.Tensor, speech_lengths: torch.Tensor, frontend_interctc: Optional[CTC] = None
        ...
        with autocast(False):
            # 1. Extract feats
            feats, feats_lengths, frontend_intermediate_outs = self._extract_feats(speech, speech_lengths, frontend_interctc)
        ...
        if intermediate_outs is not None:
            return (encoder_out, intermediate_outs), encoder_out_lens, frontend_intermediate_outs

        return encoder_out, encoder_out_lens, frontend_intermediate_outs
    ...
    def _extract_feats(
        ...
        if self.frontend is not None:
            ...
            feats, feats_lengths, frontend_intermediate_outs = self.frontend(speech, speech_lengths, frontend_interctc)
        else:
            # No frontend and no feature extract
            feats, feats_lengths = speech, speech_lengths
        return feats, feats_lengths, frontend_intermediate_outs
    ...
    def _calc_frontend_interctc_loss(
        self,
        encoder_out: torch.Tensor,
        encoder_out_lens: torch.Tensor,
        ys_pad: torch.Tensor,
        ...
        # Calc CTC loss
        loss_ctc = self.frontend_interctc(encoder_out, encoder_out_lens, ys_pad, ys_pad_lens)

        # Calc CER using CTC
        cer_ctc = None
        if not self.training and self.error_calculator is not None:
            ys_hat = self.frontend_interctc.argmax(encoder_out).data
            cer_ctc = self.error_calculator(ys_hat.cpu(), ys_pad.cpu(), is_ctc=True)
        return loss_ctc, cer_ctc
