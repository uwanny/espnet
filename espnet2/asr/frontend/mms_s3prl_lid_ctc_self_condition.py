...
class MMSS3prlFrontend(AbsFrontend):
    ...
    def forward(
        self, input: torch.Tensor, input_lengths: torch.Tensor, frontend_interctc: Optional[CTC] = None
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        feats, feats_lens = self.upstream(input, input_lengths)
        all_layer_outputs = feats
        if self.layer != -1:
            layer = self.layer
        ...
        return feats, feats_lens, all_layer_outputs
