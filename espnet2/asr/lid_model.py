...
class LIDModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if text.shape[1] != 1 and 1 in text:
            # 1 is the <unk> token, this indicates the lang in the
            # test set is not seen in the train set, like boa is not 
            # in the train set, 
            # we cut it to length 1, and replace 1 with ignore_id
            # e.g., 'abc' is not occured in the train set, so it is not 
            # occured in the token list, then the character tokenizer will
            # replace each character with <unk>, i.e. 1 in the token list, 
            # so we cut the text to length 1, only leave on <unk> token.
            text = text[:, :1]
        text[text == -1] = self.ignore_id # replace -1 with ignore_id
        ...
        if torch.isnan(speech).any():
            logging.error("NaN values found in speech")
        encoder_out = self.encode(speech, speech_lengths)

        # Check for NaN values in encoder_out
        if torch.isnan(encoder_out).any():
            logging.error("NaN values found in encoder_out")
        ...
        loss, acc = self._cal_lid_loss(encoder_out, text)
    ...
    def encode(
        ...
        with autocast(False):
            ...
            if torch.isnan(feats).any():
                logging.error("NaN values found in feats")

            # 2. Data augmentation
            if self.specaug is not None and self.training:
                feats, feats_lengths = self.specaug(feats, feats_lengths)
                if torch.isnan(feats).any():
                    logging.error("NaN values found in feats after SpecAug")

            # 3. Normalization for feature: e.g. Global-CMVN, Utterance-CMVN
            if self.normalize is not None:
                feats, feats_lengths = self.normalize(feats, feats_lengths)
                if torch.isnan(feats).any():
                    logging.error("NaN values found in feats after normalization")

        # Pre-encoder, e.g. used for raw input data
        if self.preencoder is not None:
            feats, feats_lengths = self.preencoder(feats, feats_lengths)
            if torch.isnan(feats).any():
                logging.error("NaN values found in feats after pre-encoder")
        ...
        if self.encoder.interctc_use_conditioning or getattr(
            ...
            encoder_out = self.encoder(feats, feats_lengths) # [B, Vocab_size]
        # intermediate_outs = None
        # if isinstance(encoder_out, tuple):
        #     intermediate_outs = encoder_out[1]
        #     encoder_out = encoder_out[0]
    ...
    def _cal_lid_loss(
            self, encoder_out: torch.Tensor, ys: torch.Tensor
    ):
        """
        Args:
            encoder_out: (Batch, Vocab_size)
            ys: (Batch, )
        """
        ys = ys.squeeze(-1) # [B, 1] -> [B]
        ...
        # Calculate accuracy
        preds = torch.argmax(encoder_out, dim=1)
        correct = (preds == ys).float()
        accuracy = correct.sum() / len(correct)
        
        return lid_loss, accuracy
