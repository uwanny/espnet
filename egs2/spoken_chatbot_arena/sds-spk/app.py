...
from espnet2.sds.espnet_model import ESPnetSDS_SPKModelInterface
...
print(f"current access token from env: {access_token}")
ASR_name = None
LLM_name = None
TTS_name = None
SPK_name = None
...
def parse_args():
    ...
    global registered_spk_embeddings
    ...
    parser.add_argument(
        "--spk_options",
        required=True,
        help="Provide the possible speaker embedding options available to user.",
    )
    parser.add_argument(
        "--default_asr_model",
        required=False,
    ...
    parser.add_argument(
        ...
        help="Provide the default speaker embedding model.",
    )
    parser.add_argument(
        "--upload_to_hub",
        required=False,
    ...
    if SPK_name not in SPK_options:
        print(
            "Changing default SPK model since it is "
            + "not in the possible SPK options"
        )
        SPK_name = SPK_options[0]
    upload_to_hub = args.upload_to_hub
    print(f"current access token: {access_token}")
    dialogue_model = ESPnetSDS_SPKModelInterface(
        ASR_name,
        LLM_name,
        TTS_name,
        SPK_name,
...
def start_warmup():
    ...
    for opt_count in range(len(TTS_options)):
        ...
        try:
            ...
            if opt == TTS_name:
                TTS_name = TTS_options[0]
    for opt_count in range(len(SPK_options)):
        opt = SPK_options[opt_count]
        try:
            for _ in dialogue_model.handle_SPK_selection(opt):
                continue
        except Exception as e:
            print("Removing " + opt + " from SPK options since it cannot be loaded.")
            print("Error detail:", e)
            SPK_options = SPK_options[:opt_count] + SPK_options[(opt_count + 1) :]
            if opt == SPK_name:
                SPK_name = SPK_options[0]
    ...
    for _ in dialogue_model.handle_LLM_selection(LLM_name):
        continue
    for _ in dialogue_model.handle_SPK_selection(SPK_name):
        continue
    dummy_input = (
        torch.randn(
            (3000),
...
def transcribe(
    ...
    Args:
        ...
        SPK_option: Selected SPK model option.
    ...
    if stream is None:
        # Handle user refresh
        # for loop not processed in one function call, the next call will continue from the last yield, but the function is called continuously
        for (
            _,
            _,
            _,
            ...
            type_option, TTS_option, ASR_option, LLM_option, SPK_option
        ):
    ...
    # asr_output_str is the output of the ASR model
    # text_str is the output of the LLM model
    # audio_output is the output of the TTS model
    # audio_output1 is the output of the TTS model
    # latency_ASR is the latency of the ASR model
    # latency_LM is the latency of the LLM model
    # latency_TTS is the latency of the TTS model
    # stream is the input audio stream
    # change is a flag to indicate if the output has changed
    else:
        stream = np.concatenate((stream, y))
        # print("concatenated speech and show the length of the stream", len(stream))
        print("input audio chunk length", y.shape)
    ...
    text_str1 = text_str
    print("get response from dialogue model")
    if change:
...
def register_speaker(audio):
    """
    Registers a speaker by recording their voice and storing the embedding.

    This function allows users to register their voice by recording it
    and storing the resulting speaker embedding. The registered speaker
    can then be used for speaker-based interactions in the dialogue system.

    Args:
        audio: A tuple containing:
            - `sr`: Sample rate of the audio.
            - `y`: Audio data as a numpy array.

    Returns:
        str: A message indicating the success of the speaker registration.
    """
    global registered_spk_embeddings
    global dialogue_model
    if audio is None:
    ...
    sr, y = audio
    embedding = dialogue_model.spk_model(y)
    print("embedding recorded with dimension", embedding.shape)
    if not registered_spk_embeddings:
        registered_spk_embeddings["user1"] = embedding
        dialogue_model.registered_spk_embeddings = registered_spk_embeddings
        return "Speaker 1 registered successfully!"
    else:
        registered_spk_embeddings["user" + str(len(registered_spk_embeddings) + 1)] = (
            embedding
        )
        dialogue_model.registered_spk_embeddings = registered_spk_embeddings
        return (
            "Speaker "
            + str(len(registered_spk_embeddings))
...
examples = pd.DataFrame(
    [
        ...
        [
            "General Purpose Conversation with Disfluencies",
            "Please reply to user with lot of filler words like ummm, so",
        ],
...
with gr.Blocks(
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            with gr.Row():
                SPK_radio = gr.Radio(
                    choices=SPK_options,
                    label="Choose Speaker Embedding:",
                    value=SPK_name,
                )
            with gr.Row():
                spk_register_audio = gr.Audio(
                    sources=["microphone"],
                    label="Register Your Voice",
                    type="numpy",
                    waveform_options=gr.WaveformOptions(sample_rate=16000),
                )
            with gr.Row():
                spk_register_btn = gr.Button("Register Speaker")
            with gr.Row():
                spk_register_output = gr.Textbox(
                    label="Speaker Registration Status", interactive=False
                )
            with gr.Row():
                ASR_radio = gr.Radio(
                    choices=ASR_options,
        ...
        with gr.Column(scale=1):
            ...
            output_audio = gr.Audio(
                label="Output", autoplay=True, visible=True, interactive=False
            )
            output_audio1 = gr.Audio(
                label="Output1", autoplay=False, visible=False, interactive=False
            )
    ...
    # continuous transcription, no matter if there is actual input or not
    user_audio.stream(
        transcribe,
        inputs=[
            state,
    ...
    spk_register_btn.click(
        fn=register_speaker, inputs=[spk_register_audio], outputs=[spk_register_output]
    )
    radio.change(
    ...
    type_radio.change(
        ...
        inputs=[type_radio, radio, ASR_radio, LLM_radio, SPK_radio],
        outputs=[
            radio,
            ASR_radio,
            LLM_radio,
