...
def score(references, lids, hyps):
    ...
    # there are 20 languages in few-shot setting
    few_shot_langs = [
        "[vie]", "[srp]", "[lit]", "[dan]", "[frr]", 
        "[umb]", "[tur]", "[kea]", "[tsn]", "[luo]", 
        "[ful]", "[tso]", "[kaz]", "[zul]", "[ceb]", 
    ...
    for lang in langs:
        ...
        acc = calculate_acc(lang_acc_hyps, lang_acc_refs)
        cer_avg = sum(lang_cers) / len(lang_cers)
        all_accs.append(acc)
        all_cers.append(cer_avg)  # average CER of a language
        if lang in few_shot_langs:
            few_shot_accs.append(acc)
            few_shot_cers.append(cer_avg)

    all_cers.sort(reverse=True)
    lid = round(sum(all_accs) / len(all_accs) * 100, 1)
    lid_few_shot = round(sum(few_shot_accs) / len(few_shot_accs) * 100, 1)
    cer_res = round(sum(all_cers) / len(all_cers) * 100, 1)
    cer_few_shot = round(sum(few_shot_cers) / len(few_shot_cers) * 100, 1)
    std = round(statistics.stdev(all_cers) * 100, 1)
    std_few_shot = round(statistics.stdev(few_shot_cers) * 100, 1)
    ...
    print(f"FEW SHOT AVERAGE CER: {cer_few_shot}")
    print(f"FEW SHOT CER Standard Deviation: {std_few_shot}")
    print(f"FEW SHOT LID ACCURACY: {lid_few_shot}")

    return lid, cer_res, worst, std, cer_few_shot, std_few_shot, lid_few_shot
...
if __name__ == "__main__":
    ...
    with open(f"{args.exp_dir}/challenge_results.md", "w") as out_f:
        ...
        out_f.write(
            ...
            + "CER Few Shot|CER StD Few Shot|LID Few Shot|\n"
        )
        out_f.write("|---|---|---|---|---|---|---|---|---|---|\n")
        for directory in dirs:
            if "decode_asr" in directory:
                ...
                lid, cer_res, worst, std, cer_few_shot, std_few_shot, lid_few_shot = score(
                ...
                out_f.write(
                    ...
                    + f"{cer_few_shot}|{std_few_shot}|{lid_few_shot}|\n"
                )